...
float matrix::det(void) const {
	...
	inr det = 0, M_Size = this->row - 1;
	matrix local_matr(M_Size, M_Size);
	int a = index_line, b = 0, n, m; // a,b for number
	...
	for (int i = 0; i < row; i++) {
		for (int j = 0; j < col; j++) {
			...
			for (int c = 0; c < row; c++) { //substract line
				...
				inverse_matr.ptr[j * row + c] -= (float)multiolier * inverse_matr.ptr[i*row + c];
			}
		}
	}
	return inverse_matr;
...
matrix matrix::operator *(matrix&) {
	...
	for (int i = 0; i < row; i++)
		for (int j = 0; j < col; j++)
			for (int k = 0; k < row; k++)
				result.ptr[i * row + j] += this->ptr[i*row + k] * matr.ptr[k*row + j];
	return result;
}
