...
void Shell_Sort_List( FigureList *aList )
{
	    int i, step = aList -> current_size / 2;
	while (step > 0)
    {
      for (i = 0; i < (aList->current_size - step); i++)
        {
      	  int j = i;
          while (j >= 0 && Square( &aList->circles[j] ) < Square( &aList->circles[j + step] ) )                        
            {
        		swap( &aList->circles[j], &aList->circles[j + step] );
                j--; 
            }
		}
    step /= 2; 
